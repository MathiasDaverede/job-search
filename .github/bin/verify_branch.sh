...
if [ "$target_branch" = "develop" ]; then
  if ! [[ $source_branch =~ ^feature/|^sync-v[0-9]+\.[0-9]+\.[0-9]+-to-develop$ ]]; then
    echo "Error: For target branch 'develop', the source branch name '$source_branch' does not follow the convention (feature/* or sync-vX.Y.Z-to-develop)."
    exit 1
  fi
fi
